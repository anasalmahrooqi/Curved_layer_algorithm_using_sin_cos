    ...
    for y1 in range(0, yFactor):
        ...
        for x1 in range(0, xFactor):
            ...
            if (Y1>(yFactor/2)):
                Y1 = yFactor-Y1
            # print(x1)
            f = 14*((NoOfLayers+1)/Layers)*math.sin((3.14-3.14*0.105)-(3.14-3.14*0.105*2)*((Y1*2)/(yFactor-1)))
            z1 = (NoOfLayers+1)*0.15 +(14*f/(yFactor/2))*((NoOfLayers+1)/Layers)*math.sin(abs(2.9-(3.14)*((x1*2)/(xFactor+2))))
