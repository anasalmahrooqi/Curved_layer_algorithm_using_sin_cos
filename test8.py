    ...
    for y1 in range(0, yFactor):
        ...
        for x1 in range(0, xFactor):
            ...
            if (Y1>(yFactor/2)):
                Y1 = yFactor-1-Y1
            # print(x1)
            z1 = (NoOfLayers+1)*0.15 +(3)*((NoOfLayers+1)/Layers)*pow(math.sin(abs(2.9-(3.14)*((x1)/(xFactor+2)))),2)*pow(math.sin((3.14-3.14*0.105)-(3.14-3.14*0.105*2)*((Y1)/(yFactor-1))),2)
...
for line in lines:
    ...
    for px0,py0,pz0,px1,py1,pz1 in line:
        ...
        # print(pz0 - pz00)
        # print(pz1 - pz10)
        zdis = (abs(pz0 - pz00) + abs(pz1 - pz10)) / 2
        dist = math.sqrt(pow(px1 - px0, 2) + pow(py1 - py0, 2) + pow(zdis, 2))
        area = (0.4 - zdis)*zdis+math.pi*(zdis/2)**2
        E1 += (area*dist*4)/(math.pi*1.75*1)
        # E1 += ((0.5*zdis*dist)/((math.pi*0.25*0.25)))*extrudeRate*2.5
        # E += dist * extrudeRate * (0.5 + (pz1/(maxLayerHeight*layer)))
